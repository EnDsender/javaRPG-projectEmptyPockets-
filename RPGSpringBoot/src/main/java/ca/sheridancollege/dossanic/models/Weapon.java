...
public class Weapon extends Item {
	...
	public int attack(Enemy enemy, PlayerState player) {
		...
		if (rawRoll == 20) {
			...
			System.out.println(
					"NATURAL 20! " + getName() + " critically hit " + enemy.getName() + " for " + damage + " damage!");
			return damage;
		}

		if (rawRoll == 1) {
			player.takeDamage(enemy.getBaseMinDamage());
			System.out.println("NATURAL 1! " + getName() + " fumbled! " + enemy.getName() + " counters for "
		...
		if (totalToHit >= enemy.getBaseArmorClass()) { // Make sure Enemy has getArmorClass or getBaseArmorClass
			...
			int penalty = player.getAttackDebuff();
			damage = damage - penalty;
			if (damage < 0)
				damage = 0;
			enemy.takeDamage(damage);
			System.out.println(getName() + " hit (Rolled " + rawRoll + "+" + gritModifier + "=" + totalToHit + ") for "
					+ damage + " damage.");
			return damage;
		} else {
			System.out.println(getName() + " missed! (Rolled " + rawRoll + "+" + gritModifier + "=" + totalToHit
					+ ") vs AC " + enemy.getBaseArmorClass());
			return 0;
		}
	}
}